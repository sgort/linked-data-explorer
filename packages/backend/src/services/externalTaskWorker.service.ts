 ...
 *     Writes: edocsWorkspaceId (String)
 *
 *   rip-edocs-document
 *     Reads:  edocsWorkspaceId, projectNumber, projectName
 *             documentTemplateId  (set via camunda:inputParameter in BPMN)
 *             edocsDocumentVariableName (set via camunda:inputParameter — the output variable name)
 *     Writes: <edocsDocumentVariableName> (String) — e.g. edocsIntakeReportId
 *
 * The worker converts document templates to plain-text content for upload.
 * When eDOCS integration matures, this can be replaced with a proper PDF renderer.
 *
 * Lifecycle:
 *   Call start() once during server startup. The worker runs a polling loop with
 *   Operaton's long-polling (asyncResponseTimeout). Call stop() on SIGTERM/SIGINT.
 */
export class ExternalTaskWorker {
  private readonly workerId = `lde-worker-${process.pid}`;
  private readonly operatonBaseUrl: string;
  private running = false;
  private pollTimeout: ReturnType<typeof setTimeout> | null = null;

  /** How long Operaton holds the long-poll request open (ms) */
  private readonly asyncResponseTimeout = 20_000;
  /** How long a task is locked to this worker before Operaton releases it (ms) */
  private readonly lockDuration = 60_000;
  /** Delay between poll cycles when no tasks are returned (ms) */
  ...
  private async poll(): Promise<void> {
    ...
    try {
      ...
      if (tasks.length === 0) {
        // No tasks — wait before the next cycle to avoid busy-looping
      ...
      logger.error('[ExternalTaskWorker] Poll error', { error: getErrorMessage(err) });
      // Back off briefly on errors to avoid hammering Operaton
      this.pollTimeout = setTimeout(() => void this.poll(), this.idlePollInterval);
      return;
    }

    // Immediately poll again — Operaton long-polling means this is not a busy loop
    void this.poll();
  }
      ...
      topics: [
        ...
        },
        {
          ...
          variables: [
            ...
            // Process variables used in document templates
            'confirmedScope', 'confirmedBudget', 'confirmedTimeline',
            'intakeMeetingDate', 'intakeDecisions', 'intakeAgreements',
            'psDate', 'psLocation', 'psOutcomes', 'psActionPoints',
            'psRisksIdentified', 'projectManager', 'projectSupporter',
            'environmentalManager', 'projectControlManager', 'psNotes',
            'riskFileReference', 'pdpNotes', 'assignedRoles',
            'contributorId', 'clientId', 'projectType', 'department',
          ],
        },
      ],
    };
  ...
  private async handleTask(task: ExternalTask): Promise<void> {
    logger.info('[ExternalTaskWorker] Handling task', {
      taskId: task.id,
    ...
    try {
      ...
      logger.info('[ExternalTaskWorker] Task completed', {
        taskId: task.id,
        topic: task.topicName,
        outputKeys: Object.keys(outputVariables),
      });
    } catch (err) {
      logger.error('[ExternalTaskWorker] Task failed', {
        taskId: task.id,
        topic: task.topicName,
  ...
  /**
   * rip-edocs-workspace handler
   * Creates or retrieves the eDOCS project workspace.
   */
  ...
  /**
   * rip-edocs-document handler
   * Renders a document template from process variables and uploads to eDOCS.
   *
   * documentTemplateId is set as a camunda:inputParameter on the ServiceTask in the BPMN.
   * Operaton passes input parameters as task variables with the same name.
   * edocsDocumentVariableName determines which process variable receives the document ID.
   */
  private async handleUploadDocument(
    ...
    // Build a plain-text representation of the document from available variables.
    // In a future iteration this can be replaced with a proper PDF renderer that
    // resolves the .document template stored in the Operaton deployment.
  ...
  private async failTask(taskId: string, errorMessage: string): Promise<void> {
    try {
      ...
      // Log but don't re-throw — a failure to report failure should not crash the worker
      logger.error('[ExternalTaskWorker] Failed to report task failure to Operaton', {
        taskId,
        error: getErrorMessage(err),
      });
    }
  ...
  /** Produces a minimal plain-text document body from process variables. */
