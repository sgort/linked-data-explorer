...
export function updateTestCase(
  ...
  try {
    ...
    // Update test case.
    //
    // Semgrep's insecure-object-assign flags this as a prototype-pollution risk:
    // attacker-controlled keys merged into an object. There is no input path for
    // that here. The only caller, TestCasePanel's handleLoadTestCase, passes the
    // literal { lastRun: new Date().toISOString() } -- keys fixed in source, value
    // generated by the app. The target is the user's own localStorage record, in
    // their own browser. importTestCases, the one path that takes external JSON,
    // does not reach this function.
    //
    // Revisit if updateTestCase ever receives imported or URL-supplied data: the
    // safety is a property of today's callers, not of this line.
