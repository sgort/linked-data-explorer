/**
 * packages/frontend/src/components/ShaclValidator.tsx
 *
 * Multi-file SHACL Validator — drop any number of .ttl files, validate them all
 * against the CPSV-AP 3.2.0 + RONL SHACL shapes, compare side-by-side. Cloned from
 * DmnValidator.tsx; the issue/layer rendering is identical because the backend
 * returns the same { valid, parseError, layers, summary } shape.
 *
 * Two modes:
 *   - File-local    → POST /v1/shacl/validate
 *   - Merge-simulated → POST /v1/shacl/validate-merged   (unions the file with the
 *                       already-published triples for its subjects; catches fan-out
 *                       against live data. Results depend on current published data.)
 */
...
interface ShaclValidatorProps {
  apiBaseUrl: string;
}

const ShaclValidator: React.FC<ShaclValidatorProps> = ({ apiBaseUrl }) => {
  const [entries, setEntries] = useState<ShaclEntry[]>([]);
  ...
  const addFiles = (files: FileList | File[]) => {
    ...
    const rejected = arr.filter((f) => !f.name.endsWith('.ttl'));

    if (rejected.length > 0) {
      setDropError(`Skipped ${rejected.length} file(s) — only .ttl is accepted.`);
  ...
  const validateEntry = async (id: string) => {
    ...
    const url =
      mode === 'merged'
        ? `${apiBaseUrl}/v1/shacl/validate-merged`
        : `${apiBaseUrl}/v1/shacl/validate`;
    ...
    const body =
      mode === 'merged'
        ...
        : JSON.stringify({ content: entry.content });
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-50">
      ...
      <div className="px-6 py-4 bg-white border-b border-slate-200 flex-shrink-0 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-slate-800">SHACL Validator</h2>
      ...
      <div className="flex-1 overflow-hidden flex flex-col p-6 gap-4">
          ...
          <div className="inline-flex rounded-lg border border-slate-200 bg-white p-0.5">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
                mode === 'file' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-100'
              }`}
            >
              <FileText size={13} />
              File-local
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
                mode === 'merged' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-100'
              }`}
            >
              <Layers size={13} />
              Merge-simulated
          ...
          {mode === 'merged' && (
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <input
                ...
                placeholder="SPARQL endpoint (blank = server default)"
                className="flex-1 min-w-0 px-3 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400"
              />
            </div>
          )}
        </div>
        ...
        {mode === 'merged' && (
          <p className="flex-shrink-0 -mt-2 text-xs text-slate-400">
            Merge-simulated validation unions each file with the already-published triples for its
            subjects, then validates the result. Outcomes depend on current published data.
          </p>
        )}
          ...
          <div className={`flex items-center justify-center gap-3 ${hasEntries ? '' : 'flex-col'}`}>
            ...
            {hasEntries ? (
              <span className="text-sm text-slate-500">
                Drop more files or click to browse — .ttl
              </span>
            ) : (
              <>
                <p className="text-slate-600 font-medium text-lg">Drop Turtle files here</p>
