...
const SortableChainItem: React.FC<SortableChainItemProps> = ({
  dmn,
  index,
  onRemove,
  onVendorClick,
    ...
    <div ref={setNodeRef} style={style} className="mb-3">
      ...
      >
        ...
        <div className="absolute top-3 right-3 z-10 flex items-start gap-2">
          ...
          {/* 2nd - Vendor Badge (only if count > 0) */}
          {typeof dmn.vendorCount === 'number' && dmn.vendorCount > 0 ? (
            <VendorBadge
              count={dmn.vendorCount}
              onClick={() => onVendorClick(dmn)}
              compact={true}
            />
          ) : null}

          {/* 3rd - DRD Badge (if applicable) */}
          {dmn.isDrd && (
          ...
          <button
            ...
            className="flex-shrink-0 p-1.5 rounded-full bg-white border border-slate-300 hover:border-red-300 hover:bg-red-50 transition-colors shadow-sm group"
...
const ChainComposer: React.FC<ChainComposerProps> = ({
  ...
  onClearChain,
  validation,
  endpoint, // NEW: Receive endpoint as prop
}) => {
  ...
  return (
    <>
              ...
              {chain.map((dmn, index) => {
                ...
                return (
                  <SortableChainItem
                    ...
                    onVendorClick={setVendorModalDmn} // NEW: Pass the handler
                  />
                );
              })}
            </div>
      ...
      {/* Vendor Modal - Place OUTSIDE the main div, at component level */}
      {vendorModalDmn && (
        <VendorModal
