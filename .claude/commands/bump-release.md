...
- **Then check the GitLab mirror**, which the merge has just left behind:
  ...
  reconciling. This mirror had drifted 114 commits behind on `acc` and 253 on
  `main` before its last reconciliation.
  ...
  It prints the *remote-tracking* form rather than `git push gitlab acc`:
  ...
  happens to be. This repository's local `main` sat at a four-month-old merge
  node that `origin/main` had never contained.
